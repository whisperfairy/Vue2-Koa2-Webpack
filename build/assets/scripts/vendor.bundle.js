...
/******/ 			script.src = __webpack_require__.p + "" + chunkId + ".assets/scripts/" + ({"0":"common","1":"error","2":"index-index","3":"index-post"}[chunkId]||chunkId) + ".bundle.js";
...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */(function(global, Vue) {'use strict';
	...
	var config = {
	   ...
	   * Server rendering?
	   */
	  _isServer: ({"NODE_ENV":"dev"}).VUE_ENV === 'server'
	};
	      ...
	      isObject(value) || this.deep) {
	        ...
	        if (this.user) {
	          try {
	            ...
	            ("dev") !== 'production' && warn('Error in watcher "' + this.expression + '"', this.vm);
	            /* istanbul ignore else */
	...
	function set(obj, key, val) {
	  ...
	  if (obj._isVue || ob && ob.vmCount) {
	    ("dev") !== 'production' && warn('Avoid adding reactive properties to a Vue instance or its root $data ' + 'at runtime - delcare it upfront in the data option.');
	    return;
	  }
	  if (!ob) {
	    obj[key] = val;
	...
	function initData(vm) {
	  ...
	  if (!isPlainObject(data)) {
	    data = {};
	    ("dev") !== 'production' && warn('data functions should return an object.', vm);
	  }
	  ...
	  while (i--) {
	    if (props && hasOwn(props, keys[i])) {
	      ("dev") !== 'production' && warn('The data property "' + keys[i] + '" is already declared as a prop. ' + 'Use prop default value instead.', vm);
	    } else {
	      proxy(vm, keys[i]);
	    }
	  }
	...
	function createComponent(Ctor, data, parent, context, host, _children, tag) {
	  ...
	  if (("dev") !== 'production' && _children && typeof _children !== 'function') {
	...
	function resolveAsyncComponent(factory, cb) {
	  if (factory.requested) {
	    ...
	    var _ret2 = function () {
	      ...
	      // reject
	      function (reason) {
	        ("dev") !== 'production' && warn('Failed to resolve async component: ' + factory + (reason ? '\nReason: ' + reason : ''));
	      });
	      sync = false;
	...
	function _createElement(context, tag, data, children) {
	  ...
	  if (!parent) {
	    ("dev") !== 'production' && warn('createElement cannot be called outside of component ' + 'render functions.');
	    return;
	  }
	  if (data && data.__ob__) {
	  ...
	  if (options.components) {
	    ...
	    for (var key in components) {
	      ...
	      if (isBuiltInTag(lower) || config.isReservedTag(lower)) {
	        ("dev") !== 'production' && warn('Do not use built-in or reserved HTML elements as component ' + 'id: ' + key);
	        continue;
	      }
	      def = components[key];
	...
	function getPropDefaultValue(vm, prop, name) {
	  ...
	  if (isObject(def)) {
	    ("dev") !== 'production' && warn('Invalid default value for prop "' + name + '": ' + 'Props with type Object/Array must use a factory function ' + 'to return the default value.', vm);
	  }
	...
	function query(el) {
	  if (typeof el === 'string') {
	    ...
	    if (!el) {
	      ("dev") !== 'production' && warn('Cannot find element: ' + selector);
	...
	var Transition = {
	  ...
	  render: function render(h) {
	    ...
	    // warn text nodes
	    if (("dev") !== 'production' && children.length === 1 && !children[0].tag) {
	...
	setTimeout(function () {
	  if (config.devtools) {
	    ...
	    } else if (("dev") !== 'production' && inBrowser && /Chrome\/\d+/.test(window.navigator.userAgent)) {
